...
class Camera2Session implements CameraSession {
  ...
  // -twinlife- 21016
  // keep camera request builder and callback for the zoom configuration.
    ...
    public void onOpened(CameraDevice camera) {
      ...
      try {
        camera.createCaptureSession(
                Collections.singletonList(surface), new CaptureSessionCallback(), cameraThreadHandler); // --twinlife 2025-01-31: fix style warning.
      } catch (Exception e) {
    ...
    public void onConfigured(CameraCaptureSession session) {
      ...
      try {
        ...
        session.setRepeatingRequest(
           captureRequestBuilder.build(), captureCallback, cameraThreadHandler);
        // -twinlife- 210126
      } catch (Exception e) {
  ...
  // -twinlife- 210126 add support for zoom
  @Override
  ...
  public void setZoom(final int progress) {
    Logging.d(TAG, "setZoom");
        if (cameraCharacteristics == null || captureRequestBuilder == null || captureSession == null) {
            return;
        }

        Rect rect = cameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_ACTIVE_ARRAY_SIZE);
        Float maximumZoomLevel = cameraCharacteristics.get(CameraCharacteristics.SCALER_AVAILABLE_MAX_DIGITAL_ZOOM);
        if (maximumZoomLevel == null || rect == null) {
            return;
        }

        int minWidth = (int) (rect.width() / maximumZoomLevel);
        int minHeight = (int) (rect.height() / maximumZoomLevel);
        int diffWidth = rect.width() - minWidth;
        int diffHeight = rect.height() - minHeight;
        int croppedWidth = (diffWidth * progress) / 100;
        int croppedHeight = (diffHeight * progress) / 100;
        if (croppedWidth < 0 || croppedHeight < 0 || croppedWidth > diffWidth || croppedHeight > diffHeight) {
            return;
        }
        ...
        try {
            ...
            reportError("Failed to set the zoom request. " + e);
        }
  }
  // -twinlife- 210126
